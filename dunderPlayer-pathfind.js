...
var doFindingPath = function(bot, objStuff, extension, correction, endX, endY, endZ, maxAttempts, extraOptions) {
        ...
        while (!bot.dunder.foundPath && bot.dunder[objStuff.attempts] < 7500 && (process.hrtime(bot.dunder.performanceStop)[0] * 1000000000 + process.hrtime(bot.dunder.performanceStop)[1]) / 1000000 < 40 / botsToSpawn.length) {
            ...
            if (bot.dunder[objStuff.bestNode] && (bot.dunder[objStuff.attempts] > bot.dunder[objStuff.maxAttempts] /*&& dist3d(bot.dunder[objStuff.bestNode].x, bot.dunder[objStuff.bestNode].y, bot.dunder[objStuff.bestNode].z, bot.dunder.goal.x, bot.dunder.goal.y, bot.dunder.goal.z) < dist3d(bot.dunder[objStuff.lastPos].x, bot.dunder[objStuff.lastPos].y, bot.dunder[objStuff.lastPos].z, bot.dunder.goal.x, bot.dunder.goal.y, bot.dunder.goal.z)*/ || endZ != undefined && /*bot.dunder[objStuff.bestNode].x == endX && bot.dunder[objStuff.bestNode].y == endY && bot.dunder[objStuff.bestNode].z == endZ*/ distMan3d(bot.dunder[objStuff.bestNode].x, bot.dunder[objStuff.bestNode].y, bot.dunder[objStuff.bestNode].z, endX, endY, endZ) <= bot.dunder[objStuff.pathGoalForgiveness] && (!extraOptions || extraOptions && !extraOptions.mustBeVisible || extraOptions && extraOptions.mustBeVisible && visibleFromPos(bot, new Vec3(bot.dunder[objStuff.bestNode].x, bot.dunder[objStuff.bestNode].y, bot.dunder[objStuff.bestNode].z), new Vec3 (endX, endY, endZ), bot.dunder[objStuff.bestNode])) ||
        ...
        if (!bot.dunder[objStuff.bestNode] || bot.dunder.foundPath || maxAttempts >= 7500 /*|| botPathfindTimer > 20 * 3*/) {
            ...
            if (bot.dunder[objStuff.attempts] > bot.dunder[objStuff.maxAttempts]) {
               ...
               if (objStuff.failedPathfind) {
                   bot.dunderTaskDetails[objStuff.failedPathfind] = {x:endX, y:endY, z:endZ};
               }
               bot.dunder[objStuff.movesToGo] = [];
