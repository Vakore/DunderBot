...
const dunderBotPathfindDefaults = {
    ...
    "pathGoalForgiveness":"pathGoalForgiveness",
    "failedPathfind":"failedPathfind",
};
const dunderBotPathfindSkips = {
    ...
    "pathGoalForgiveness":"skipPathGoalForgiveness",
};
...
const crafter = require("mineflayer-crafting-util").plugin//Big thanks to Gen for this
...
eval(fs.readFileSync(__dirname + '\\dunderPlayer-miningtask.js')+'');
...
function makeBots(cbtm) {
    ...
    bots[cbtm].once("spawn", () => {
        bots[cbtm].loadPlugin(crafter)
        //bots[cbtm].physics.autoJumpCooldown = 0;
    ...
    bots[cbtm].on("physicsTick", () => {
        if (bots[cbtm].dunder.spawned) {
            bots[cbtm].jumpTicks = 0;
            //console.log(bots[cbtm].jumpTicks);
            ...
            if (bot.targetDigBlock) {//low priority look if nothing else is being done
                botLookAt(bot, bot.targetDigBlock.position, 1);
            }
...
/*TODO:
make blaze goal closer to blaze when blaze is higher in the air/closer to zombies when on hills
maintain distance against skeletons/strays holding melee weapons
...
if (false) {
    ...
    */
    ...
    } else if (bot.dunder.state == "on_fire" && hasItem(bot, [bot.dunder.bucketTask.bucket/*, "axolotl_bucket"*/])) {
