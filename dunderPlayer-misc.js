...
function digBlock(bot, x, y, z) {
    ...
    if (canMine && !bot.targetDigBlock) {
        ...
        bot.dig(bot.blockAt(new Vec3(x, y, z)), true).catch(e => {});
    }
};
...
function placeBlock(bot, x, y, z, placeBackwards, extraOptions = {}) {
    ...
    if (canPlace) {
        ...
        botLookAt(bot, new Vec3(x, y, z), 200);
        //attackTimer = 0;
        console.log("asdf 1");
...
function visibleFromPos(bot, pos1, pos2, zeNode) {
    //console.log(entity.position.offset(0, entity.height / 2, 0).minus(bot.entity.position.offset(0, 1.6, 0)).normalize());
    let theRaycast = bot.world.raycast(pos1.offset(0, 1.62, 0), pos2.offset(0.5, 0.5, 0.5).minus(pos1.offset(0, 1.62, 0)).normalize(), 5);
    if (theRaycast && theRaycast.position) {
...
function placeCraftingTable(bot, wantedBlock = "crafting_table") {
            ...
            if (placeLocation.length > 0) {
                ...
                placeBlock(bot, placeLocation[0], placeLocation[1], placeLocation[2], false, {useBlocks:[wantedBlock],endFunc:function() {console.log("finished");bot.dunderTaskCompleted = true;} });
                //setTimeout( () => {}, 100);
